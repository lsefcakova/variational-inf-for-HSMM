...
#         uf = np.zeros((self.K,self.K))
#         uf[0,1:] = self.mod_init[1:]
#         self.mod_init_B_tilde = expand_matrix(uf, self.prior_expansion_param,self.prior_expansion_lengths)[0,]
...
#         self.var_x_exp = np.exp(self.lalpha + self.lbeta) #14 in log self.var_x = q*(x_t=k)
#         ind_tracker=0
...
#         # ##print(self.var_x_exp)
#         # # ##print('shpL',self.var_x_exp.shape)
#         # ind_tracker=0
#         # for i in range(len(self.prior_expansion_lengths)):
#         #     self.var_x[:,i] = np.sum(self.var_x_exp[:,ind_tracker:(ind_tracker + self.prior_expansion_lengths[i])],axis=1)
#         #     ind_tracker = ind_tracker + self.prior_expansion_lengths[i]

#         # ##print(self.var_x_exp.shape)

#         # ##print('old:;',self.var_x)

#         # self.var_x -= np.max(self.var_x, axis=1)[:,npa] 
#         # #why do we substract the max value? is this step 15??
#         # ##print('new:;',self.var_x)
...
#         # ##print('lliks:',self.lliks)
#         self.lliks_exp = np.zeros((self.T,self.M))
#         # ##print('hi')
#         for i in range(self.lliks.shape[0]):
#             row_i = []
#             for j in range(len(self.lliks[i])):
#                 row_i = row_i + self.prior_expansion_lengths[j]*[self.lliks[i][j]]
#             # ##print(len(row_i))
#             self.lliks_exp[i,:] = row_i
            
        
#         ll = self.lliks_exp

#         lalpha = self.lalpha
#         # ##print('alpha: ',self.mod_init)
