...
from hmmbase import VariationalHSMMBase
...
class VBHSMM(VariationalHSMMBase):
    ...
    def __init__(self, obs, prior_init, prior_tran, prior_emit,
                 prior_expansion_param,prior_expansion_lengths, mask=None,
        ...
        super(VBHSMM, self).__init__(obs, prior_init, prior_tran, prior_emit,
                                    prior_expansion_param,prior_expansion_lengths,
        ...
        self.lalpha = np.empty((self.T, self.M))
        self.lbeta = np.empty((self.T, self.M))
...
# chganged this from the initial formulation which used atol (early termination of algorithm before convergence)
    ...
    def global_update(self):
        ...
        for t in range(1, self.T):
            ...
            # hyperparams dirichle row (alphas) + the quantities expe vale by takign  the first
            # sum in the eqs and they do one by the other one instead of doing the actual joint 
        np.fill_diagonal(self.var_tran,0)
        print(self.var_tran)
