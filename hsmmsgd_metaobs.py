...
class VBHSMM(VariationalHSMMBase):
    ...
    def __init__(self, obs, prior_pi0, prior_A, prior_emit,m_js,lambda_js,
                 tau=tau0, kappa=kappa0, metaobs_half=metaobs_half0, mb_sz=mb_sz0, mask=None,
        ...
        super(VBHSMM, self).__init__(obs, prior_pi0, prior_A, prior_emit,
                                     m_js,lambda_js, mask=mask, init_init=init_init,
        ...
        if adagrad:
            self.ada_G = 1.0*np.ones(prior_A.shape)  #initialize with 1's the weight matrix
        ...
        self.alpha_table = np.zeros((metaobs_sz, self.M))
        self.beta_table = np.zeros((metaobs_sz, self.M))
        self.c_table = np.zeros(metaobs_sz)
        
        #self.lalpha = np.empty((metaobs_sz, self.K))
        #self.lbeta = np.empty((metaobs_sz, self.K))
        self.lliks = np.empty((metaobs_sz, self.K))
        
        self.lalpha_tilde = np.empty((metaobs_sz, self.M))
        self.lbeta_tilde = np.empty((metaobs_sz, self.M))
        self.lliks_tilde = np.empty((metaobs_sz, self.M))
    ...
    def local_lower_bound(self):
        ...
        return np.sum(np.logaddexp.reduce(self.lalpha_tilde, axis=1))
    ...
    def infer(self, adaptive=False, perIter=10, epsilon=1e-6, minHalfL=1,
        ...
        for it in range(maxit):
            ...
            #SKIP: for now we don't do adaptive
            ...
            #SKIP: for now we don't do growBuffer
            ...
            A_inter = np.zeros_like(self.var_A)
            
            #init emit
            emit_inter = [util.NIW_zero_nat_pars(self.var_emit[0])
                          for k in range(K)]
            
            #in one iteration, we visit several sets of minibatches (mb_sz value)
            for data in minibatch:
                ...
                # passes.
                A_mean = self.var_A / np.sum(self.var_A, axis=1)[:,npa]
...
## till here all good? no local_update and global_update
...
###
        ...
        for t in range(loff+1,uoff+1):
            #lalpha[t-loff] = np.logaddexp.reduce(lalpha[t-loff-1] + ltran.T, axis=1) + ll[t-loff]
            self.lalpha_tilde[t-loff] = np.logaddexp.reduce(lalpha_tilde[t-loff-1] + ltran_ext.T, axis=1) + ll_ext[t-loff]
...
###
        ...
        self.lbeta_tilde[L-1,:] = 0. #log of 1 
###

        for t in reversed(range(loff, uoff)):
            #np.logaddexp.reduce(ltran + lbeta[t-loff+1,:] + ll[t-loff+1], axis=1,
            #                    out=lbeta[t-loff,:])
            np.logaddexp.reduce(ltran_ext + lbeta_tilde[t-loff+1] + ll_ext[t-loff+1], axis=1,
                                out=self.lbeta_tilde[t-loff])
    ...
    def global_update(self, A_inter, emit_inter):
        ...
        if type(self.var_emit[0]) is Gaussian:
            for k in range(self.K):
                ...
                nats_new = (1.-lrate)*nats_old + lrate*(prior_hypparam + bfact*emit_inter[k])
