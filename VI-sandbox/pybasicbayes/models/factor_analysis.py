...
class _FactorAnalysisBase(Model, metaclass=abc.ABCMeta):
...
class _FactorAnalysisGibbs(_FactorAnalysisBase, ModelGibbsSampling, metaclass=abc.ABCMeta):
...
class _FactorAnalysisMeanField(_FactorAnalysisBase, ModelMeanField, ModelMeanFieldSVI, metaclass=abc.ABCMeta):
    def _null_stats(self):
        return objarray(
            [np.zeros(self.D_obs),
             np.zeros((self.D_obs, self.D_latent)),
