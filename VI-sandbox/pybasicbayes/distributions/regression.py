...
class Regression(GibbsSampling, MeanField, MaxLikelihood):
    ...
    def _check_shapes(A, sigma, nu, S, M, K):
        ...
        assert all(map(is_2d, list(filter(not_none, [A, sigma, S, M, K])))), 'Matrices must be 2D'

        get_dim = lambda x, i: x.shape[i] if x is not None else None
        get_dim_list = lambda pairs: list(filter(not_none, list(map(get_dim, *list(zip(*pairs))))))
